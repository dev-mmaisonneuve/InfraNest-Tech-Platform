...
export async function sendNotificationEmail({ subject, heading, rows }: EmailParams) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 680px; margin: 0 auto; color: #0f172a;">
      <h1 style="font-size: 24px; margin-bottom: 20px;">${heading}</h1>
      <table style="width: 100%; border-collapse: collapse;">
        ${rows
          .map(
            ([label, value]) => `
              <tr>
                <td style="padding: 10px 12px; border: 1px solid #dbe3ef; background: #f8fbff; width: 180px; font-weight: 700;">${label}</td>
                <td style="padding: 10px 12px; border: 1px solid #dbe3ef;">${value}</td>
              </tr>
            `,
          )
          .join("")}
